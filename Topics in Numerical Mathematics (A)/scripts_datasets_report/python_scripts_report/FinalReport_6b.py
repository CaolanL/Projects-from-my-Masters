# Comparison of the accuracy of the 5 scoring approaches:
# -1: Frobenius Error
...
sampling_methods = [
    ...
    (efficient_probabilities_exact_leverage, "exact statistical", 0.1),
    (efficient_probabilities_exact_lambda_ridge, "exact lambda-ridge", 0),
    (efficient_probabilities_approx_lambda_ridge, "approx lambda-ridge", 0.1)
]

# Set-up
nr_epsilons = 25
epsilons = np.linspace(0.001, 0.2, nr_epsilons)  #fraction of total n columns to be sampled: s = epsilon * n

for ds_idx, (dataset_name, dataset_title, kernel, lambd, n, test_rate, integer) in enumerate(datasets):
    all_errors_K = np.zeros((len(sampling_methods), nr_epsilons)) #2D-array to store all errors
    ...
    for m_idx, (sampling_method, sampling_name, p_factor) in enumerate(sampling_methods):
        ...
        for e_idx, epsilon in enumerate(epsilons):
            ...
            all_errors_K[m_idx, e_idx] = (np.linalg.norm(K_exact - K_approx, ord = 'fro'))

    # Plotting
    sampled_columns = n * epsilons
    plt.title(f"Approximating $K$ for {dataset_title}")
    plt.semilogy(sampled_columns, all_errors_K[0,:], marker='.', label="Uniform")
    plt.semilogy(sampled_columns, all_errors_K[1,:], marker='.', label="Diagonal")
    plt.semilogy(sampled_columns, all_errors_K[2,:], marker='.', label="Exact leverage")
    plt.semilogy(sampled_columns, all_errors_K[3,:], marker='.', label="Exact lambda-ridge")
    plt.semilogy(sampled_columns, all_errors_K[4,:], marker='.', label="Approx lambda-ridge")
